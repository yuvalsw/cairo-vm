...
impl Into<HintError> for CairoPieLoaderError {
...
fn build_cairo_pie_memory_map(memory: &CairoPieMemory) -> HashMap<Relocatable, &MaybeRelocatable> {
    ...
    for ((segment_index, offset), value) in memory {
...
fn relocate_cairo_pie_memory(
    ...
    for ((segment_index, offset), value) in &cairo_pie.memory {
        let address = Relocatable::from((*segment_index as isize, *offset));
        ...
        vm.segments
            .memory
            .insert(relocated_address, relocated_value)?;
    }
